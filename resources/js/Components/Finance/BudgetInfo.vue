...
function deleteBudget(id) {
    if (
        confirm(
            "Are you sure you want to delete this budget and subbudgets?\nYou cannot recover it once deleted"
        )
    ) {
        Inertia.delete(`budget/${id}`);
    }
}
...
<template>
    <div class="mt-4 flex justify-between items-center w-full">
        <div class="flex flex-row text-left">
            ...
            <span class="basis-1/4"
                >${{ props.budget.progress }}/{{ props.budget.amount }}</span
            >
        </div>

        <div class="flex flex-row lg:w-5/12 w-1/3 justify-between">
            <button
                class="hover:text-blue-500 border border-blue-500 hover:underline py-2 px-4 rounded-2xl hover:bg-white duration-150 ease-in-out"
                @click="() => open()"
            >
            ...
            <button
                v-if="name != 'Income'"
                class="hover:text-red-500 border border-red-500 hover:underline py-2 px-4 rounded-2xl hover:bg-white duration-150 ease-in-out"
                @click="deleteBudget(id)"
            >
    ...
    <transition name="slide-fade">
        <div
            v-if="showSubBudgets"
            className="bg-white rounded-xl border-2 my-4 px-4 transition-all ease-in-out"
            :style="{ borderColor: color }"
        >
            <button
                class="hover:text-green-400 py-2 pr-4 rounded-2xl hover:bg-white duration-150 ease-in-out"
                @click="() => openAdd()"
            >
                Sub Budgets <i class="fa-solid fa-plus"></i>
            </button>
            <div
                v-for="budget in subbudgets"
